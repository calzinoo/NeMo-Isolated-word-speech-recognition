...
def slice_folder(class_name):
    ...
    for file in os.listdir(folder_path):
        ...
        for i, chunk in enumerate(chunks):
            ...
            if len(chunk) < config.TARGET_LEN_MS:
                silence_duration = config.TARGET_LEN_MS - len(chunk)
                silence = AudioSegment.silent(duration=silence_duration)
                chunk = chunk + silence
            else:
                chunk = chunk[:config.TARGET_LEN_MS] # Taglia se troppo lungo
            
            # Esporta in formato NeMo (16kHz, Mono, WAV)
            chunk = chunk.set_frame_rate(config.SAMPLE_RATE).set_channels(1)
