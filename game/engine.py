    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if game_over:
                    ...
                    if event.key == pygame.K_UP and player.state != "JUMPING":
                        player.state = "JUMPING"
                        player.timer = config.JUMP_DURATION
                        
                    if event.key == pygame.K_DOWN and player.state != "DUCKING":
                        player.state = "DUCKING"
                        player.timer = config.DUCK_DURATION
                        
                    if event.key == pygame.K_SPACE and player.state != "SMASHING": 
        ...
        if not game_over:
            ...
            if spawn_timer <= 0:
                ...
                spawn_timer = max(50, 130 - (score // 10))
            ...
            for obs in obstacles[:]:
                ...
                if player.rect.colliderect(obs.rect):
                    # Se è un muro e player sta spaccando, e il muro non è già rotto...
                    ...
                    # Se il muro è già rotto, passa attraverso senza morire
        ...
        # Disegna prima monete, poi ostacoli, poi giocatore
        for c in coins:
            c.draw(screen)
